...
function computeRisk(i) {
  ...
  const marginDollars = i.equity * (i.riskPct / 100);
  const notional = marginDollars * i.leverage;
  const maxLossAtStop = notional * (i.stopPct / 100);
  ...
  return {
    ...
    liqStopRatio,
    riskScore: score,
    dollarRisk: maxLossAtStop,
    // consumers (UI, post-trade modal) use this name
    entry
  };
}
var state = {
...
function checkRules(action) {
  ...
  if (state.cooldownUntil && Date.now() < state.cooldownUntil) {
    const hours = ((state.cooldownUntil - Date.now()) / 36e5).toFixed(1);
    return {
      rule: "loss_cooldown",
...
var WALLET_KEY = "hyperguard:wallet";
var ARBITRUM = {
  chainId: "0xa4b1",
...
var _localDate = (t) => new Date(t * 1e3);
var _tickMarkFormatter = (time, tickMarkType) => {
  const d = _localDate(time);
  switch (tickMarkType) {
    case 0:
      return d.toLocaleDateString(void 0, { year: "numeric" });
    case 1:
...
var _crosshairTimeFormatter = (time) => {
  const d = _localDate(time);
  const subday = _tfHasSubday();
  return d.toLocaleString(void 0, subday ? { month: "short", day: "2-digit", hour: "2-digit", minute: "2-digit" } : { year: "numeric", month: "short", day: "2-digit" });
};
...
var candleSeries = chart.addCandlestickSeries({
  upColor: "#a3f7bf",
  downColor: "#ff7468",
  borderUpColor: "#a3f7bf",
  borderDownColor: "#ff7468",
  wickUpColor: "#a3f7bf",
  wickDownColor: "#ff7468"
});
var candles = [];
var liveSource = "pending";
var HL_REST = "https://api.hyperliquid.xyz/info";
var HL_WS = "wss://api.hyperliquid.xyz/ws";
var marketsCache = { byCoin: (
  /** @type {Record<string,any>} */
  {}
) };
var currentCoin = "BTC";
var FAV_KEY = "hyperguard:favorites";
function loadFavorites() {
  ...
  return /* @__PURE__ */ new Set(["BTC", "ETH", "HYPE"]);
}
...
function formatPx(p) {
  ...
  if (p >= 1e3) return p.toLocaleString("en-US", { maximumFractionDigits: 1 });
...
function fundingCountdown() {
  ...
  const ms = 36e5 - (now.getUTCMinutes() * 6e4 + now.getUTCSeconds() * 1e3 + now.getUTCMilliseconds());
...
async function fetchMetaAndCtxs() {
  ...
  meta.universe.forEach((u, i) => {
    marketsCache.byCoin[u.name] = ctxs[i];
  });
  return { meta, ctxs };
}
...
function renderDropdown() {
  ...
  const all = Object.keys(marketsCache.byCoin).filter((c) => !filterRaw || c.includes(filterRaw)).map((c) => ({ coin: c, ctx: marketsCache.byCoin[c] }));
  const favs = all.filter(({ coin }) => favorites.has(coin)).sort((a, b) => +b.ctx.dayNtlVlm - +a.ctx.dayNtlVlm);
  const others = all.filter(({ coin }) => !favorites.has(coin)).sort((a, b) => +b.ctx.dayNtlVlm - +a.ctx.dayNtlVlm).slice(0, 80);
  let html = "";
  if (favs.length) {
...
var INTERVAL_SECONDS_BY_KEY = { "15m": 15 * 60, "4h": 4 * 3600, "1d": 86400, "1w": 7 * 86400 };
var CURRENT_INTERVAL = "15m";
var INTERVAL_SECONDS = INTERVAL_SECONDS_BY_KEY[CURRENT_INTERVAL];
...
async function loadHyperliquidHistory(coin = currentCoin) {
  ...
  const startTime = endTime - 200 * INTERVAL_SECONDS * 1e3;
  const res = await fetch(HL_REST, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
function openHyperliquidStream(coin = currentCoin) {
  ...
  ws.onclose = () => {
    ...
    const delay = Math.min(3e4, 1e3 * 2 ** _wsRetry);
...
function seedSynthetic() {
  ...
  for (let k = 0; k < 200; k++) {
    ...
    const drift = (Math.random() - 0.5) * 35e-4;
    const open = lastClose;
    const close = open * (1 + drift);
    const hi = Math.max(open, close) * (1 + Math.random() * 15e-4);
    const lo = Math.min(open, close) * (1 - Math.random() * 15e-4);
...
function syntheticTick() {
  ...
  const last = candles[candles.length - 1];
  const drift = (Math.random() - 0.5) * 12e-4;
...
function updateOhlc(bar) {
  ...
  const $o = document.getElementById("oh-o");
  if ($o) $o.textContent = formatPx(o);
  const $h = document.getElementById("oh-h");
  if ($h) $h.textContent = formatPx(h);
  const $l = document.getElementById("oh-l");
  if ($l) $l.textContent = formatPx(l);
  const $c = document.getElementById("oh-c");
  if ($c) $c.textContent = formatPx(c);
...
var fmt0 = (n) => "$" + n.toLocaleString("en-US", { maximumFractionDigits: 0 });
function clampInputs() {
  ...
  $("levHint").textContent = `cap ${m.maxLeverage}\xD7`;
  $("riskHint").textContent = `${m.maxRiskPct}% max \xB7 ${m.label}`;
}
function recompute() {
  const inputs = {
    equity: state.equity,
  ...
  if (newBand !== _prevRiskBand) {
    const escalating = _prevRiskBand === "safe" && newBand !== "safe" || _prevRiskBand === "warn" && newBand === "danger";
...
function pollRules() {
  ...
  const r3 = $("rh-3");
  if (state.cooldownUntil && Date.now() < state.cooldownUntil) {
    r3.classList.remove("ok");
    r3.classList.add("tripped");
    const h = ((state.cooldownUntil - Date.now()) / 36e5).toFixed(1);
    r3.querySelector(".rv").textContent = `${h}h left`;
  } else {
    r3.classList.add("ok");
    r3.classList.remove("tripped");
  ...
  if (violation) {
    ...
    placeBtn.textContent = "Blocked \xB7 " + violation.rule.replace(/_/g, " ");
...
$("placeBtn").addEventListener("click", () => {
  if (state.position) {
    toast("Close current position first", "bad");
    return;
  }
  const violation = checkRules({ type: "placeOrder", side: state.side });
  if (violation) {
...
window.tryMoveStop = tryMoveStop;
var _pcKey = (p) => p ? p.coin + ":" + p.side + ":" + p.leverage + ":" + p.notional + ":" + p.openedAt : "";
var _pcMountedKey = "";
var _pcLastTpState = null;
var fmtSigned$ = (n) => (n >= 0 ? "+" : "\u2212") + "$" + Math.abs(n).toFixed(2);
function mountPosCard(p) {
    ...
    <div class="pos-card ${p.side}">
      <div class="ph-grid">
        <div class="pcell headline">
          <div class="k">${p.coin || currentCoin}-USDC \xB7 ${p.side.toUpperCase()} \xB7 ${p.leverage}\xD7 \xB7 ${fmt0(p.notional)}</div>
        ...
        <div class="pcell" id="pc-tp-cell"><span class="k" style="color:var(--mint);">\u{1F4A1} Click chart to set TP</span><span class="v" style="color:var(--text-3);">\u2014 not set</span></div>
  ...
  $("pc-fees").textContent = "\u2212$" + (p.notional * 1e-3).toFixed(2);
  _pcLastTpState = null;
}
function renderPosCard() {
  ...
  const totalFee = p.notional * 1e-3;
  const hourlyRate = +(marketsCache.byCoin[p.coin]?.funding || 0);
  const heldHours = Math.max(0, (Date.now() - p.openedAt) / 36e5);
  ...
  const stopAtBE = Math.abs(p.stopPrice - p.entry) / p.entry < 5e-4;
  ...
  $("pc-fund-k").textContent = `Funding \xB7 ${(hourlyRate * 100).toFixed(4)}%/h`;
  const fund = $("pc-fund");
  ...
  if (beBtn) {
    ...
    beBtn.title = stopAtBE ? "Stop already at break-even" : !inProfit ? "Available once trade is in profit" : "Move stop to entry \u2014 locks zero loss";
  }
  ...
  if (wantState !== _pcLastTpState) {
    if (wantState === "set") {
      ...
      tpCell.innerHTML = `
        <span class="k" style="color:var(--mint);">\u{1F4A1} Click chart to set TP</span>
...
function closePosition(reason) {
  ...
  const feesUsd = p.notional * 1e-3;
  const heldHours = (Date.now() - p.openedAt) / 36e5;
  const fundingUsd = heldHours > 1 ? -p.notional * 1e-4 * Math.floor(heldHours) : 0;
  ...
  toast(`${reasonText} \xB7 ${pnl >= 0 ? "+" : ""}${fmt$(pnl)}`, pnl >= 0 ? "ok" : "bad");
}
...
$("walletBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (addr) {
      ...
      toast(`${provider} connected \xB7 no custody`, "ok");
...
chart.timeScale().subscribeVisibleTimeRangeChange(() => {
  const r = state.position ? { stopPrice: state.position.stopPrice, liqPrice: state.position.liqPrice } : (() => {
    const x = recompute();
    return x;
  })();
  paintRiskZone(r.stopPrice, r.liqPrice);
